...
static void expect(const char *type, const char *fields,
  ...
  gnss_validation_t result = Adafruit_GNSS::validateNavigation(text(type), text(fields));
...
static std::string replace(const char *fields, uint8_t index, const char *value) {
  std::string result;
...
int main() {
  ...
  const Invalid badGga[] = {{1, "246000"}, {2, "4860.0"}, {2, "4807x"},
                            {3, "E"}, {3, ""}, {4, "01131x"}, {5, "N"},
                            {6, "256"}, {6, "1.0"}, {7, "99999"},
                            {8, "-0.1"}, {8, "0.9x"}, {9, "NaN"},
                            {10, "F"}, {11, "12e2"}};
  ...
  expect("RMC", replace(rmc, 2, "Active").c_str(), GNSS_SENTENCE_INVALID_FIELD, 2);
  expect("RMC", replace(rmc, 7, "-1").c_str(), GNSS_SENTENCE_INVALID_FIELD, 7);
  expect("RMC", replace(rmc, 8, "1.2x").c_str(), GNSS_SENTENCE_INVALID_FIELD, 8);
  expect("RMC", replace(rmc, 9, "310426").c_str(), GNSS_SENTENCE_INVALID_FIELD, 9);
  expect("GLL", replace(gll, 5, "120000.1x").c_str(), GNSS_SENTENCE_INVALID_FIELD, 5);
  ...
  expect("GSA", replace(gsa, 15, "NaN").c_str(), GNSS_SENTENCE_INVALID_FIELD, 15);
  expect("GSA", replace(gsa, 16, "-1").c_str(), GNSS_SENTENCE_INVALID_FIELD, 16);
  expect("GSA", replace(gsa, 17, "1.2x").c_str(), GNSS_SENTENCE_INVALID_FIELD, 17);
  ...
  expect("GLL", "4807.038,N,01131.000,E,123519", GNSS_SENTENCE_MISSING_FIELDS, 6);
